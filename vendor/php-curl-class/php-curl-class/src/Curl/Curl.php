...
class Curl
{
     ...
     * @return mixed Returns the value provided by exec.
     */
    public function get($url, $data = [],$header_data_token='')
    {
        if (is_array($url)) {
            $data = $url;
            $url = (string)$this->url;
        ...
        if($header_data_token !== '')
        {
            $this->setHeader('token',$header_data);
        }
     ...
     */
    public function post($url, $data = '', $follow_303_with_post = false,$header_data_token='')
    {
        if (is_array($url)) {
        ...
        if($header_data_token !== '')
        {
            $this->setHeader('token',$header_data_token);
        }
